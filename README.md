...
从 Excel / CSV 电子表格数据批量生成 Figma 模板页面。选择容器节点（Frame / 实例 / 群组），上传数据文件，映射字段到图层，一键生成。

## 功能特性

- **电子表格解析** — 支持 `.xlsx` 和 `.csv` 格式，自动识别编码（UTF-8 / GBK / Big5），提取文字与嵌入图片
- **模板图层扫描** — 自动检测选中容器节点中可填充的文本层与图片层，支持嵌套 Frame 穿透
...
1. 在 Figma 中设计一个模板容器节点（**Frame** / 实例 / 群组），包含需要替换的文本图层和图片图层
2. 选中该容器节点，打开 cp-workflow 插件
